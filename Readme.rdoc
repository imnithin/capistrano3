...
* Although application would be running fine, You may run into strange and common error - When you run rails commands(rails -v from current dir) "The program 'rails' can be found in the following packages: ....." Solution: gem install rails -v 3.2.19 from current directory. I'm believing that it searches in .rvm directory of specified ruby version, but during cap deployment gems will be installed into shared directory.
